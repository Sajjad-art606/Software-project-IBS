...
function StepLinks({ links }: { links: NonNullable<GuideStep['links']> }) {
  return (
...
export function GuideStepsList({
  steps,
  slug,
  interactive = false,
}: {
  ...
  const storageKey = slug ? `guide-progress-${slug}` : ''
  const [checked, setChecked] = useState<Set<number>>(() => new Set())

  useEffect(() => {
    if (!interactive || !storageKey) return
    try {
  ...
  const progress =
    interactive && steps.length > 0 ? Math.round((checked.size / steps.length) * 100) : 0
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ol className="flex flex-col gap-4">
        {steps.map((step) => {
          const isDone = interactive && checked.has(step.id)
          return (
            <li
              key={step.id}
